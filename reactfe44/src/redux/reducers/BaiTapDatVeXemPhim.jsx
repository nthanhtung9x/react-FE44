import * as type from '../actions/types/DatVeXemPhimType';
...
export default (state = initialState, action) => {
    switch (action.type) {
        case type.HANDLE_CHANGE: {
            let temp = [...state.danhSachChon];
            let index = temp.findIndex(item => item.ghe.soGhe === action.ghe.soGhe);
            if(index !== -1) {
                temp.splice(index, 1);
            } else {
                temp.push({hang: action.hang, ghe: action.ghe});
            }
            state.total = temp.reduce((total, item) => {
                return total += item.ghe.gia;
            },0);

            state.danhSachChon = temp;
            return {...state};
        }    
        case type.DELETE_TICKET: {
            let temp = [...state.danhSachChon];
            let index = temp.findIndex(item => item.ghe.soGhe = action.soGhe);
            temp.splice(index,1);
            state.danhSachChon = temp;
            state.total = temp.reduce((total, item) => {
                return total += item.ghe.gia;
        ...
        case type.BOOK_TICKET: {
            ...
            let danhSachChon = [...state.danhSachChon];
            if(danhSachChon.length > 0) {
                for(let i = 0; i < danhSachChon.length; i++) {
                    for(let j = 0; j < danhSach.length; j++) {
                        if(danhSachChon[i].hang === danhSach[j].hang) {
                            let danhSachGhe = danhSach[j].danhSachGhe;
                            for(let k = 0; k < danhSachGhe.length; k++) {
                                if(danhSachChon[i].ghe.soGhe === danhSachGhe[k].soGhe) {
                                    danhSachGhe[k].daDat = true;
                                    break;
                                }
                            }
                        }
